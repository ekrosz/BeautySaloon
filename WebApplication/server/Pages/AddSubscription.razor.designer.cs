...
namespace WebApplication.Pages
{
    public partial class AddSubscriptionComponent : ComponentBase
    {
        ...
        {
            subscription = new WebApplication.Models.LocalDb.Subscription(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(WebApplication.Models.LocalDb.Subscription args)
        {
            try
            {
                var localDbCreateSubscriptionResult = await LocalDb.CreateSubscription(subscription);
                DialogService.Close(subscription);
            }
            catch (System.Exception localDbCreateSubscriptionException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new Subscription!" });
            }
        }
